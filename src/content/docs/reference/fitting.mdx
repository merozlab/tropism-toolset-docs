...
**Parameters:**
- `data` (pd.DataFrame): DataFrame from `x_y_to_s_theta` with 's' and 'theta' columns
- `rotate` (str or None, optional): Rotation option: "horizontal", "vertical", or None. Default: None
...
- `display` (bool, optional): Display fit plot. Default: False
...
# Get steady-state frame data and convert to s-theta
frame_data = data[data['frame'] == steady_frame]
s_theta_data = x_y_to_s_theta(frame_data)
