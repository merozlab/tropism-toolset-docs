...
>
To perform tip angle analysis, you need the following experimental data:
  <DataItem
    name="Centerline coordinates"
    value="CSV files with x, y pixel positions and frame numbers for each time point"
    required
  />
  <DataItem
    name="Pixel-to-meter conversion"
    value="Spatial calibration factor for coordinate transformation"
    units="px/m"
    required
  />
  <DataItem
    name="Frame interval"
    value="Time between consecutive frames (default: 15 minutes)"
    units="minutes/frame"
    required
  />
  <DataItem
    name="Tip percentage"
    value="Percentage of centerline to use for tip angle calculation (default: 20%)"
    units="%"
  />
  <DataItem
    name="Time limit"
    value="Maximum time window for analysis (default: 120 hours)"
    units="hours"
  />
</RequiredData>
